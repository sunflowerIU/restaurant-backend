...
export async function seed() {
  try {
    ...
    //get id of category
    const lunchId = categories?.find((c) => c.name === "lunch")?._id;
    const dinnerId = categories?.find((c) => c.name === "dinner")?._id;
    const drinksId = categories?.find((c) => c.name === "drinks")?._id;
    const dessertId = categories?.find((c) => c.name === "dessert")?._id;
    ...
    await Product.insertMany(fakeProducts).catch((err) => console.log(err));
